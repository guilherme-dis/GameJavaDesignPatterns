...
public class SimplePersonagemFactory {
    public static Personagem createPersonagem(int x, int y) {
        ...
        if (n <= 0.2) {
            ...
            p = new Personagem2("Guerreiro", x, y);
            p.setAtacar(new Raio(p.getAtacar()));
            p.setAtacar(new Tiro(p.getAtacar()));
            p.setAtacar(new SuperChute(p.getAtacar()));
        } else if (0.4 < n || n <= 0.6) {

            p = new Personagem3("Guerreiro", x, y);
            p.setAtacar(new Tiro(p.getAtacar()));
            p.setAtacar(new SuperChute(p.getAtacar()));
            p.setCorrer(new CorridaDevagar());
        } else if (0.6 < n || n <= 0.8) {

            p = new Personagem4("Guerreiro", x, y);
            p.setAtacar(new Raio(p.getAtacar()));
            p.setAtacar(new Tiro(p.getAtacar()));
            p.setPular(new PuloBaixo());
        } else if (0.8 < n || n <= 1.0) {

            p = new Personagem5("Guerreiro", x, y);
            p.setCorrer(new CorridaDevagar());
